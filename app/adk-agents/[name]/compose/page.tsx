...
export default function ADKAgentComposePage() {
  ...
  const loadFiles = useCallback(async (resetOriginal = true) => {
    ...
    try {
      ...
      const validationResponse = await fetch(`/api/adk-agents/${agentName}/validate`);
  ...
  const handleNodeDelete = useCallback(async (nodeId: string, filename: string | undefined): Promise<boolean> => {
    ...
    try {
      const response = await fetch(`/api/adk-agents/${agentName}/files?filename=${encodeURIComponent(filename)}`, {
        method: 'DELETE',
      });
  ...
  const handleAgentDroppedInContainer = useCallback(async (containerId: string, containerFilename: string, droppedAgentFilename: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/adk-agents/${agentName}/connections`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleRemoveFromContainer = useCallback(async (
    ...
    try {
      ...
      const response = await fetch(`/api/adk-agents/${agentName}/connections?parentFilename=${encodeURIComponent(containerFilename)}&childFilename=${encodeURIComponent(childFilename)}`, {
        method: 'DELETE',
      });
